    ...
    function findEXIFinJPEG(oFile) {
      ...
      while (iOffset < iLength) {
        if (oFile.getByteAt(iOffset) != 0xff) {
          if (bDebug)
            console.log(
              'Not a valid marker at offset ' + iOffset + ', found: ' + oFile.getByteAt(iOffset)
            );
    ...
    function readTagValue(oFile, iEntryOffset, iTIFFStart, iDirStart, bBigEnd) {
      ...
      switch (iType) {
        ...
        case 10: // signed rational, two slongs, first is numerator, second is denominator
          if (iNumValues == 1) {
            return (
              oFile.getSLongAt(iValueOffset, bBigEnd) / oFile.getSLongAt(iValueOffset + 4, bBigEnd)
            );
          } else {
            var aVals = [];
            for (var n = 0; n < iNumValues; n++) {
...
$(function () {
  var someCallback = function (exifObject) {
    ...
    $('#GPSLAT').val(exifObject.GPSLatitude);
    $('#GPSLON').val(exifObject.GPSLongitude);
    console.log('exifObject', exifObject);
